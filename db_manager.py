...
class DBManager:
	...
	def create_data_table(self, data_name):
		...
		cursor.execute('CREATE TABLE {tn} ({nf_id} {ft_i} PRIMARY KEY {nn}, {nf_date} {ft_i} {nn}, {nf_high} {ft_r} {nn}, {nf_low} {ft_r} {nn}, {nf_open} {ft_r} {nn}, {nf_close} {ft_r} {nn}, {nf_volume} {ft_r} {nn}, {nf_qVol} {ft_r} {nn}, {nf_wAvg} {ft_r} {nn})'\
				.format(tn = data_name, nf_id = self.col_id, nf_date = self.col_date, nf_high = self.col_high, nf_low = self.col_low, nf_open = self.col_open, nf_close = self.col_close, nf_volume = self.col_volume, nf_qVol = self.col_quoteVolume, nf_wAvg = self.col_weightedAverage, ft_i = self.ft_integer, ft_r = self.ft_real, nn = "NOT NULL"))
		self.conn.commit()
		self.conn.close()
